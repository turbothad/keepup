...
import { StyleSheet, FlatList, ActivityIndicator, RefreshControl, TouchableOpacity, View, Alert } from 'react-native';
...
export default function FeedScreen() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      const sortedPosts = fetchedPosts.sort((a: ApiPost, b: ApiPost) => 
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );
      setPosts(sortedPosts);
    } catch (error) {
      console.error('Error fetching posts:', error);
      if (error && typeof error === 'object' && 'message' in error && error.message === 'No token, authorization denied') {
  ...
  const displayedPosts = showFriendsOnly
    ? (user?.friends 
        ? posts.filter(post => {
            ...
            const isFriend = user.friends.some((friendId: string) => 
              friendId === postUserId || 
              friendId === postUserId.toString()
            );
            
            // Debug logging for each post's user ID and whether they're a friend
            console.log(`Post by user ${post.user.username} (ID: ${postUserId}): ${isFriend ? 'Is friend' : 'Not a friend'}`);
  ...
  if (authError) {
    return (
      <ThemedView style={[styles.container, styles.centered]}>
        <ThemedText type="subtitle" style={styles.errorText}>{authError}</ThemedText>
        <TouchableOpacity 
